  ...
  return (
    <div className='flex flex-col gap-4'>
      <label className='mb-1 block font-medium'>
        Leistungen wählen für {category}:
      </label>
        ...
        {services.map((item) => {
          ...
          return (
            <div key={key} className='flex flex-col gap-1'>
              <label
                className={`flex cursor-pointer items-center gap-3 rounded border px-3 py-2 transition-colors duration-150 ${checked ? 'border-[#bb9167] bg-[#f7f2ee]' : 'border-neutral-200 bg-white hover:bg-neutral-50'}`}
              >
                <CustomCheckbox
                  checked={checked}
                ...
                <span className='ml-auto font-semibold text-[#bb9167] select-none'>
                  {item.price}
                </span>
              </label>
              {checked &&
                item.additionalPrices &&
                item.additionalPrices.length > 0 && (
                  <div className='ml-8 flex flex-col gap-1'>
                    {item.additionalPrices.map((addon) => (
                      <label
                        key={addon.name}
                        className='flex items-center gap-2 rounded border border-[#ecdcc7] bg-[#f7f2ee] px-2 py-1 text-sm'
                      >
                        <CustomCheckbox
                          checked={getSelectedAddonsForService(item).includes(
                            addon.name
                          )}
                          onChange={() => onAddonChange(item, addon.name)}
                        />
                        <span>{addon.name}</span>
                        <span className='ml-auto font-semibold text-[#bb9167]'>
                          {addon.price}
                        </span>
                      </label>
                    ))}
